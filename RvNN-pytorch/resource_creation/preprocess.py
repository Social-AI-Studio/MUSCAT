        ...
        for line in mainfile:
            if line:
                ...
                if not items[1] in alltrees:
                    alltrees.add(items[1])
                outputfile.write(items[0]+"\t"+"FILLER"+"\t"+items[1]+"\t\n")
                
ratio = 0.7
alltrees = list(alltrees)
with open("test_fold1.txt","w",encoding="utf-8") as outfile:
    for item in alltrees[:int(len(alltrees)*(1-ratio))]:
        outfile.write(str(item)+"\n")
with open("train_fold1.txt","w",encoding="utf-8") as outfile:
    for item in alltrees[int(len(alltrees)*(1-ratio)):]:
        outfile.write(str(item)+"\n")