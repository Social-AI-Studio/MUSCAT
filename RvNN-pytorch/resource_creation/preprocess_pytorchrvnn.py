...
alllabels = []
unverified = []
with open(os.path.join("PHEME","label.txt"),"r",encoding="utf-8") as mainfile:
    with open("falsified_labels.txt","w",encoding="utf-8") as outputfile:
        for line in mainfile:
            if line:
                items = line.strip().split(":")
                if not items[1] in alltrees:
                    if items[0]=="unverified" or items[0]=="non-rumor":
                        unverified.append(items[1])
                    else:
                        alltrees.append(items[1])
                        alllabels.append(items[0])
                outputfile.write(items[0]+"\t"+"FILLER"+"\t"+items[1]+"\t\n")
                
ratio = 0.7


holder = list(zip(alltrees,alllabels))
random.shuffle(holder)
alltrees,alllabels = zip(*holder)
alllabels_number = []
for i in alllabels:
    if i=="false":
        alllabels_number.append(0)
    elif i=="true":
        alllabels_number.append(1)
splitter = StratifiedShuffleSplit(n_splits=1, test_size=0.3, random_state=0)
random.seed(0)
counter=1
for train_index,test_index in splitter.split(alltrees,alllabels_number):
    random.shuffle(unverified)
    with open("train_PHEME"+str(counter)+".txt","w",encoding="utf-8") as outfile:
        helditems = []
        for item in train_index:
            helditems.append(alltrees[item])
        for item in unverified[:int(len(unverified)*0.5)]:
            helditems.append(item)
        random.shuffle(helditems)
        for item in helditems:
            outfile.write(item+"\n")    
        
    with open("test_PHEME"+str(counter)+".txt","w",encoding="utf-8") as outfile:
        helditems = []
