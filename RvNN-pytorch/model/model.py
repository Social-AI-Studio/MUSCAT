...
def _getTreeInfo(tree):
    ...
    # logger.info(f"leaves_idx --> {leaves_idx}, {len(idx_dict)}")
    return {
        "num_node": len(idx_dict),
        "num_leaf": len(leaves_idx),
...
class RvNN(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size, num_class):
        ...
        self.E = nn.Parameter(
            torch.normal(
                mean=torch.zeros(self.hidden_size, self.vocab_size, requires_grad=True),
                std=0.1,
            )
        )
    ...
    def forward(self, tree, word, word_idx):
        ...
        h_list = torch.zeros(
            tree_info["num_node"], self.hidden_size, requires_grad=True
        ).to(device)
        ...
        # logger.info(f"leaves --> {tree_info['leaves_idx']}")
