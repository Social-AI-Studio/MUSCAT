...
for epoch in range(epoches):
    ...
    print(
        "epoch={}: loss={:.6f}, takes {:.2f}s".format(
            epoch, np.mean(losses), time.time() - t_s
        )
    )
    ...
    if epoch % 5 == 0:
        ...
        print(
            "Loss after num_examples_seen={}, epoch={}, loss={:.6f}".format(
                num_examples_seen, epoch, np.mean(losses)
            )
        )
        ...
        for test_idx in range(len(y_test)):
            pred = model.forward(
                tree_train[test_idx], word_train[test_idx], index_train[test_idx]
            )
        ...
        if len(losses_5) > 1 and losses_5[-1][1] > losses_5[-2][1]:
            ...
            model_optimizer = torch.optim.SGD(
                params=model.parameters(), momentum=0.9, lr=lr
            )
