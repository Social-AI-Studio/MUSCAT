...
df = pd.read_csv("VietFactCheck_csvdump.csv")
print(df.head())
...
for index, row in df.iterrows():
    ...
    author = row["Author"]
    date = parser.parse(row["date"]).isoformat()
    # print(date)
    ...
    if lang != "discard":
        content_dict = {"title": title,
                        ...
                        "category": category,
                        "language": lang}
        with open(os.path.join("viet_articles", "{}.json".format(str(index).zfill(4))), "w") as handle:
