        ...
        while True:
            try:
                ...
                if len(saved_data) >= requested_number or force_end:
                    ...
                    with open(os.path.join(additional_directory, query + ".json"), "w",
...
if __name__ == "__main__":
    ...
    # df = pd.read_csv("tweets_counted_full_with_ids.tsv", sep='\t', )
    # df = df.loc[df['results_count'] != 0]
    # print("number of articles: ")
    # print(len(df))
    # # print (df.iloc[98:300])

    root_dir = "convo_search_results"
    for event in os.listdir(root_dir):
        convo_id_list = []
        for file in os.listdir(os.path.join(root_dir, event)):
            root_id = file.split(".")[0]
            convo_id_list.append(root_id)

        filename = event
        if len(convo_id_list) > 0:
    ...
    #     # print(keywords_str)
    #
    #     data_path = os.path.join("convo_search_results",
    #                              keywords_str)
    #     if os.path.exists(data_path):
    #         print(os.listdir(data_path))
    #         root_tweet_ids = [i.split(".")[0] for i in os.listdir(data_path)]
    #
    ...
    #
    #     search_list.append([keywords_str, root_tweet_ids])
    #
    # print(search_list)
    ...
    for i in search_list:
        path_to_save = os.path.join("root_tweets_results", i[0])
        ...
        for hashtag in i[1]:
            ...
            print(os.path.join(path_to_save, hashtag + ".json"))
            if os.path.exists(os.path.join(path_to_save, hashtag + ".json")):
                print(os.path.join(path_to_save, hashtag + ".json"), "was done..")
                # no_of_results.append(0)
                continue
            # print(i)
            else:
                output, *_ = instance.archive_search(hashtag, requested_number=100,
