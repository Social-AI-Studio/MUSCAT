...
kw_model = KeyBERT()
kw_model_multi = KeyBERT(model="distiluse-base-multilingual-cased-v2")

rootdir = "thai_articles"
for file in os.listdir(rootdir):
    with open(os.path.join(rootdir, file), 'r') as f:
        ...
        # print(title)
            ...
            translated_title = translate_text_with_source(source="th", target="en", text=title)
            content["title_eng"] = translated_title
            # print(title)
            # print(translated_title)
            keywords_eng = kw_model.extract_keywords(translated_title, keyphrase_ngram_range=(1, 1), top_n=5)
            # print(keywords_thai)
            selected_keywords = [i[0] for i in keywords_eng]
            content["keywords_eng"] = selected_keywords
            keywords_thai = [translate_text_with_source(source="en", target="th", text=word) for word in selected_keywords]
            content["keywords"] = keywords_thai
            # keywords = kw_model.extract_keywords(translated_title, stop_words="english", keyphrase_ngram_range=(1, 1), top_n=5)
            # sorted_keywords = sorted(keywords, key=lambda tup: tup[1], reverse=True)
            # # print(sorted_keywords)
            # selected_keywords = [i[0] for i in keywords]
            # content["keywords_eng"] = selected_keywords
            # keywords_thai = [translate_text_with_source(source="en", target="vi", text=word) for word in selected_keywords]
            # content["keywords"] = keywords_thai

            path_to_save = rootdir + "_with_keywords_v2"
            if not os.path.exists(path_to_save):
                os.mkdir(path_to_save)
