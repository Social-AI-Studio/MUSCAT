        ...
        while True:
            try:
                ...
                if len(saved_data) >= requested_number or force_end:
                    ...
                    with open(os.path.join(additional_directory,
                                           query + "_" + "_".join(starttime.split("-")[:2]) + ".json"), "w",
                              encoding="utf-8") as milkteacheck:  # remove
...
if __name__ == "__main__":
    ...
    root_dir = "thai_articles_with_keywords_v2"
    root_save_dir = "thai_raw_results_v2"

    for file in os.listdir(root_dir):
        with open(os.path.join(root_dir, file), 'r') as f:
            content = json.load(f)

        #wtf, missing date in a few articles
        #RESOLVED
        try:
            post_time = datetime.datetime.fromisoformat(content['date'].strip())
            print(post_time)
        except:
            print(file)
            print(content['date'])
        ...
        '''
        Some articles has only 1 or 2 keywords
        This is because the entire title is literally filled with stopwords
        E.g. "To mask or not to mask"
        Those will be disqualified for search
        '''
        if len(keywords) >1:
            keywords_list.append([[start_date_str, end_date_str, root_save_dir], [keywords_str]])


    # df = pd.read_csv("indo_with_first_para_translated.tsv", encoding='utf-8', sep="\t", skiprows=range(1, 4517))
    #
    ...
    #     print(keywords_str)
    #
    #     keywords_list.append([[start_date_str, end_date_str, "indo_search_results"], [keywords_str]])
        # print(start_date_str)

        # print(start_date, end_date)
    # convo_id = 1425421657502593033

    # keywords_list = [[["2020-04-01T00:01:00-08:00", "2021-11-05T00:01:00-08:00", "test_convo"],
    #                   ["conversation_id:{}".format(convo_id)]]]
    ...
    print("pulling", len(keywords_list), " events.")
    for i in keywords_list:
        if not os.path.exists(i[0][2]):
            os.mkdir(i[0][2])
        filelist = os.listdir(i[0][2])
        for hashtag in i[1]:
            print("attempting: ", hashtag)
            # print(os.path.join("MILKTEA_SEGREGATED","#MilkTeaAlliance"+"_"+"_".join(i[0][0].split("-")[:2])+".json"))
            print(
                os.path.join(root_save_dir, hashtag.strip() + "_" + "_".join(i[0][0].split("-")[:2]) + ".json"))
            if os.path.exists(os.path.join(root_save_dir,
                                           hashtag.strip() + "_" + "_".join(i[0][0].split("-")[:2]) + ".json")):
                print(os.path.join(root_save_dir, hashtag + "_" + "_".join(i[0][0].split("-")[:2]) + ".json"),
                      "was done..")
                no_of_results.append(0)
                continue
            # print(i)
            output, savedToken, number_of_tweets = instance.archive_search(hashtag, starttime=i[0][0], endtime=i[0][1],
                                                                           requested_number=100,
                                                                           additional_directory=i[0][2])
