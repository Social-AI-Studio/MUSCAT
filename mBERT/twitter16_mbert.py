...
for LEARNING_RATE in LEARNING_RATE_LIST:
    ...
    # 28 Feb edit Dan: now able to train on one language and test on another
    train_lang = "en"
    test_lang = "en"


    # test_event_list = ['sydneysiege-all-rnr-threads']
    ...
    for split in range(5):
        ...
        root_dir_train = "processed_twitter16_{}".format(train_lang)
        root_dir_test = "processed_twitter16_{}".format(test_lang)
        with open(os.path.join(root_dir_test, "twitter16_split_{}_test_content".format(split)), "rb") as handle:
            X_test = pickle.load(handle)
        with open(os.path.join(root_dir_test, "twitter16_split_{}_test_label".format(split)), "rb") as handle:
            y_test_raw = pickle.load(handle)

        # X_train = []
        # y_train_raw = []

        with open(os.path.join(root_dir_test, "twitter16_split_{}_train_content".format(split)), "rb") as handle:
            X_train = pickle.load(handle)
        with open(os.path.join(root_dir_test, "twitter16_split_{}_train_label".format(split)), "rb") as handle:
        ...
        torch.save(model.state_dict(), "mbert_twitter16_state_dict_{}_{}.pth".format(train_lang, event))
        ...
        with open("preds_temp_run2_temp_{}.pickle".format(event), 'wb') as handle:
    ...
    with open("preds_{}_{}_twitter16_mbertcased_lr{}.pickle".format(train_lang, test_lang, LEARNING_RATE), 'wb') as handle:
