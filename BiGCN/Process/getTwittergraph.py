...
def constructMat(tree):
    ...
    for index_i in range(len(index2node)):
        for index_j in range(len(index2node)):
            if (
                index2node[index_i + 1].children != None
                and index2node[index_j + 1] in index2node[index_i + 1].children
            ):
...
def main(obj, lang):
    treePath = os.path.join(cwd, "data/" + obj, lang + "/data.TD_RvNN.vol_5000.txt")
    ...
    Parallel(n_jobs=30, backend="threading")(
        delayed(loadEid)(treeDic[eid] if eid in treeDic else None, eid, labelDic[eid])
