...
class RumorProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, fold):
        """See base class."""
        lines = []
        for lang in ["en", "th", "vi", "id", "ms"]:
            logger.info(
                "LOOKING AT {}".format(os.path.join(data_dir, lang, fold, "train.tsv"))
            )
            _lines = self._create_examples(
                self._read_tsv(os.path.join(data_dir, lang, fold, "train.tsv")), "train"
            )
            lines.extend(_lines)
        return lines
    ...
    def get_dev_examples(self, data_dir, fold):
        ...
        for lang in ["en", "th", "vi", "id", "ms"]:
            logger.info(
                "LOOKING AT {}".format(os.path.join(data_dir, lang, fold, "dev.tsv"))
            )
            _lines = self._create_examples(
                self._read_tsv(os.path.join(data_dir, lang, fold, "dev.tsv")), "dev"
            )
            lines.extend(_lines)
        return lines
    ...
    def get_test_examples(self, data_dir, fold, lang):
        """See base class."""
        logger.info(
            "LOOKING AT {}".format(os.path.join(data_dir, lang, fold, "test.tsv"))
        )
...
def get_dataset(args, split_type, tokenizer, lang=None):
    ...
    if split_type == "train":
        examples = processor.get_train_examples(args.data_dir, args.fold)
    elif split_type == "dev":
        examples = processor.get_dev_examples(args.data_dir, args.fold)
    else:
        examples = processor.get_test_examples(args.data_dir, args.fold, lang)
...
def main():
    ...
    parser.add_argument(
        "--fold",
        default="charliehebdo",
        type=str,
        help="The name of the fold to train.",
    )
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        for lang in ["en", "id", "vi", "th", "ms"]:
            eval_data = get_dataset(args, "test", tokenizer, lang)
            ...
            fout_p = open(
                os.path.join(args.output_dir, lang, args.fold, "pred.txt"), "w"
            )
            fout_t = open(
                os.path.join(args.output_dir, lang, args.fold, "true.txt"), "w"
            )
            ...
            output_eval_file = os.path.join(
                args.output_dir, lang, args.fold, "eval_results.json"
            )
