...
class CoInputFeatures(InputFeatures):
    def __init__(self, input_ids1, input_mask1, segment_ids1, input_ids2, input_mask2, segment_ids2,
                 input_ids3, input_mask3, segment_ids3, input_ids4, input_mask4, segment_ids4, input_mask,
                 label_id, stance_position, src_input_ids, src_input_mask):
        self.input_ids1 = input_ids1
        self.input_mask1 = input_mask1
        self.segment_ids1 = segment_ids1
        self.input_ids2 = input_ids2
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features.append(
            CoInputFeatures(input_ids1=input_ids1, input_mask1=input_mask1, segment_ids1=segment_ids1,
                                ...
                                src_input_ids=src_input_ids, src_input_mask=src_input_mask))
    return features
...
def main():
    ...
    if args.task_name.lower() == "pheme4cls":
        args.num_train_epochs = 7
    ...
    if args.do_train:
        ...
        all_src_input_ids = torch.tensor([f.src_input_ids for f in train_features], dtype=torch.long)
        all_src_input_mask = torch.tensor([f.src_input_mask for f in train_features], dtype=torch.long)
        train_data = TensorDataset(all_input_ids1, all_input_mask1, all_segment_ids1,
                                   ...
                                   all_input_mask, all_src_input_ids, all_src_input_mask, all_label_ids)
        if args.local_rank == -1:
        ...
        all_src_input_ids = torch.tensor([f.src_input_ids for f in eval_features], dtype=torch.long)
        all_src_input_mask = torch.tensor([f.src_input_mask for f in eval_features], dtype=torch.long)
        ...
        all_src_input_ids = torch.tensor([f.src_input_ids for f in test_features], dtype=torch.long)
        all_src_input_mask = torch.tensor([f.src_input_mask for f in test_features], dtype=torch.long)
        ...
        for train_idx in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_mask, src_input_ids, src_input_mask, label_ids = batch
                loss = model(input_ids1, segment_ids1, input_mask1, input_ids2, segment_ids2, input_mask2,
                             ...
                             input_mask, src_input_ids, src_input_mask, label_ids)
                if n_gpu > 1:
            ...
            for input_ids1, input_mask1, segment_ids1, input_ids2, input_mask2, segment_ids2, \
                input_ids3, input_mask3, segment_ids3, input_ids4, input_mask4, segment_ids4, \
                input_mask, src_input_ids, src_input_mask, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
                input_ids1 = input_ids1.to(device)
                input_mask1 = input_mask1.to(device)
                segment_ids1 = segment_ids1.to(device)
                input_ids2 = input_ids2.to(device)
                ...
                with torch.no_grad():
                    ...
                    logits = model(input_ids1, segment_ids1, input_mask1, input_ids2, segment_ids2, input_mask2,
                                   input_ids3, segment_ids3, input_mask3, input_ids4, segment_ids4, input_mask4,
                                   input_mask, src_input_ids, src_input_mask)
            ...
            for input_ids1, input_mask1, segment_ids1, input_ids2, input_mask2, segment_ids2, \
                input_ids3, input_mask3, segment_ids3, input_ids4, input_mask4, segment_ids4, \
                input_mask, src_input_ids, src_input_mask, label_ids in tqdm(test_dataloader, desc="Evaluating"):
                input_ids1 = input_ids1.to(device)
                input_mask1 = input_mask1.to(device)
                segment_ids1 = segment_ids1.to(device)
                input_ids2 = input_ids2.to(device)
